    ...
    <form method="post" action="">
        <div class="register-form-wrap">
            ...
            <?php if (houzez_option('register_mobile', 0) == 1) { ?>
                <div class="form-group">
                    <div class="form-group-field phone-field">
                        <input class="form-control" id="phone_number" name="phone_number" type="tel" maxlength="10" placeholder="<?php esc_html_e('Phone', 'houzez') ?>" required />
                    </div><!-- input-group -->
                </div><!-- form-group -->
            <?php } ?>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        phoneInput.addEventListener("input", function () {
            ...
            // Nếu dài hơn 11 số thì cắt bớt
            else if (this.value.length > 11) {
                this.value = this.value.slice(0, 11);
                hideError();
            }
            // Nếu đang nhập mà không sai thì ẩn lỗi
            else {
                hideError();
            }
        });
        ...
        phoneInput.addEventListener("blur", function () {
            if (!/^0\d{9,10}$/.test(this.value)) {
                showError("Số điện thoại phải từ 10–11 số và bắt đầu bằng 0");
            } else {
                hideError();
            }
        });
